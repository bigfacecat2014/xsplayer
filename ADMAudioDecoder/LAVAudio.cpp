...
CLAVAudio::CLAVAudio(LPUNKNOWN pUnk, HRESULT* phr)
  : CTransformFilter(TEXT("lavc audio decoder"), 0, __uuidof(CLAVAudio))
{
  ...
  m_pOutput = new CTransformOutputPin(TEXT("CTransformOutputPin"), this, phr, L"Output");
  if(!m_pOutput) {
    *phr = E_OUTOFMEMORY;
  }
  if(FAILED(*phr))  {
