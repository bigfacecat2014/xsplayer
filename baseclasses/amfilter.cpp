...
)
{
    ...
    if (m_Connected) {
        hr = SetMediaType((CMediaType*)pmt); // hack by yxs. 用于支持动态媒体格式改变。
        return VFW_E_ALREADY_CONNECTED;
    }
...
{
    CUnknown* pUnkRet = new CMemAllocator(TEXT("CMemAllocator"), pUnk, phr);
    return pUnkRet;
}
    ...
    */
    ...
    for (; m_lAllocated < m_lCount; m_lAllocated++, pNext += lAlignedSize) {
        ...
        pSample = new CMediaSample(
            TEXT("Default memory media sample"),
            this,
            &hr,
