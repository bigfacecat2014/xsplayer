...
#endif
        OutputDebugString(psz); // add by yxs
    } else {
...
void WINAPI DisplayType(LPCTSTR label, const AM_MEDIA_TYPE *pmtIn)
{
    ...
    ASSERT(pmtIn->formattype != FORMAT_VideoInfo); // ADM已彻底淘汰第一代视频信息头结构体
    if (pmtIn->formattype == FORMAT_VideoInfo2) {
        VIDEOINFOHEADER2* vih2 = (VIDEOINFOHEADER2*)pmtIn->pbFormat;
        DisplayRECT(TEXT("Source rectangle"), vih2->rcSource);
        DisplayRECT(TEXT("Target rectangle"), vih2->rcTarget);
        DbgLog((LOG_TRACE, 5, TEXT("Aspect Ratio: %d:%d"), vih2->dwPictAspectRatioX, vih2->dwPictAspectRatioY));
        DisplayBITMAPINFO(&vih2->bmiHeader);
    }
        ...
        ASSERT(pmtIn->subtype != MEDIASUBTYPE_MPEG1Packet); // ADM杜绝一切淘汰的技术标准和格式。
