...
const LONGLONG REF_TIME_MILLISECONDS = (1000);            // 1秒有10 ^ 3个毫秒
const LONGLONG REF_TIME_NANOSECONDS = (1000000000);       // 1秒有10 ^ 9个纳秒
const LONGLONG UNITS_IN_100NS = (REF_TIME_NANOSECONDS / 100);      // 1秒有10 ^ 7个百纳秒

/*  Unfortunately an inline function here generates a call to __allmul
    - even for constants!
*/
#define MILLISECONDS_TO_100NS_UNITS(lMs) \
    Int32x32To64((lMs), (UNITS_IN_100NS / REF_TIME_MILLISECONDS))
...
public:
    ...
    {
        return (LONG)(m_time / (UNITS_IN_100NS / REF_TIME_MILLISECONDS));
    };
