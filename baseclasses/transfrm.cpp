...
CBasePin* CTransformFilter::GetPin(int n)
{
    ...
    if (m_pInput == NULL) {
        m_pInput = new CTransformInputPin(TEXT("Transform input pin"), this, &hr, L"XForm In");
        ASSERT(SUCCEEDED(hr));
        if (m_pInput == NULL) {
            return NULL;
        }
        m_pOutput = (CTransformOutputPin*)new CTransformOutputPin(TEXT("Transform output pin"), this, &hr, L"XForm Out");
        ASSERT(SUCCEEDED(hr));
        if (m_pOutput == NULL) {
            delete m_pInput;
            m_pInput = NULL;
...
HRESULT CTransformFilter::StartStreaming()
{
    return NOERROR;
}


HRESULT CTransformFilter::StopStreaming()
{
    return NOERROR;
}
...
HRESULT CTransformFilter::CheckConnect(PIN_DIRECTION dir, IPin* pPin)
{
...
HRESULT CTransformFilter::BreakConnect(PIN_DIRECTION dir)
{
    UNREFERENCED_PARAMETER(dir);
    return NOERROR;
}
...
HRESULT CTransformFilter::CompleteConnect(PIN_DIRECTION direction, IPin* pReceivePin)
{
...
HRESULT CTransformFilter::SetMediaType(PIN_DIRECTION direction, const CMediaType* pmt)
{
    UNREFERENCED_PARAMETER(direction);
...
HRESULT CTransformFilter::InitializeOutputSample(IMediaSample* pSample, __deref_out IMediaSample** ppOutSample)
{
...
HRESULT CTransformFilter::Receive(IMediaSample* pSample)
{
...
HRESULT CTransformFilter::EndOfStream(void)
{
    HRESULT hr = NOERROR;
    if (m_pOutput != NULL) {
...
HRESULT CTransformFilter::BeginFlush(void)
{
    HRESULT hr = NOERROR;
    if (m_pOutput != NULL) {
...
// or a worker thread
HRESULT CTransformFilter::EndFlush(void)
{
...
STDMETHODIMP CTransformFilter::Stop()
{
...
STDMETHODIMP CTransformFilter::Pause()
{
...
HRESULT CTransformFilter::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop, double dRate)
{
    if (m_pOutput != NULL) {
...
HRESULT CTransformInputPin::CheckStreaming()
{
...
HRESULT CTransformInputPin::CheckConnect(IPin* pPin)
{
...
HRESULT CTransformInputPin::BreakConnect()
{
...
HRESULT CTransformInputPin::CompleteConnect(IPin* pReceivePin)
{
...
HRESULT CTransformInputPin::CheckMediaType(const CMediaType* pmt)
{
...
HRESULT CTransformInputPin::SetMediaType(const CMediaType* mtIn)
{
...
STDMETHODIMP CTransformOutputPin::NonDelegatingQueryInterface(REFIID riid, __deref_out void** ppv)
{
...
HRESULT CTransformOutputPin::CheckMediaType(const CMediaType* pmtOut)
{
...
HRESULT CTransformOutputPin::SetMediaType(const CMediaType* pmtOut)
{
    HRESULT hr = NOERROR;
    ASSERT(m_pTransformFilter->m_pInput != NULL);
...
HRESULT CTransformOutputPin::DecideBufferSize(IMemAllocator* pAllocator, __inout ALLOCATOR_PROPERTIES* pProp)
{
    return m_pTransformFilter->DecideBufferSize(pAllocator, pProp);
}

HRESULT CTransformOutputPin::GetMediaType(int iPosition, __inout CMediaType* pMediaType)
{
    ASSERT(m_pTransformFilter->m_pInput != NULL);
...
STDMETHODIMP CTransformOutputPin::Notify(IBaseFilter* pSender, Quality q)
{
