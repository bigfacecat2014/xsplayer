...
// =================================================================
CTransInPlaceFilter::CTransInPlaceFilter( __in_opt LPCTSTR pName, __inout_opt LPUNKNOWN  pUnk,
...
CBasePin* CTransInPlaceFilter::GetPin(int n)
{
    ...
    if (m_pInput == NULL) {
        ...
        m_pInput = new CTransInPlaceInputPin(TEXT(""), this, &hr, L"Input");
        ASSERT(SUCCEEDED(hr));
    }
    ...
    if (m_pInput!=NULL && m_pOutput == NULL) {
        ...
        m_pOutput = new CTransInPlaceOutputPin( TEXT(""), this, &hr, L"Output");
        ASSERT(SUCCEEDED(hr));
        if (m_pOutput == NULL) {
            delete m_pInput;
            m_pInput = NULL;
...
HRESULT CTransInPlaceFilter::Receive(IMediaSample *pSample)
{
...
// =================================================================
CTransInPlaceInputPin::CTransInPlaceInputPin( __in_opt LPCTSTR pObjectName,
    __inout CTransInPlaceFilter* pFilter, __inout HRESULT* phr, __in_opt LPCWSTR pName)
...
STDMETHODIMP CTransInPlaceInputPin::NotifyAllocator(IMemAllocator* pAllocator, BOOL bReadOnly)
{
    HRESULT hr = S_OK;
...
STDMETHODIMP CTransInPlaceInputPin::GetAllocatorRequirements(__out ALLOCATOR_PROPERTIES *pProps)
{
    if( m_pTIPFilter->m_pOutput->IsConnected() )
...
HRESULT CTransInPlaceInputPin::CompleteConnect(IPin *pReceivePin)
{
