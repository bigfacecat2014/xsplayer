...
, m_hThread(0)
{
    ...
    else
    {
        ...
        m_rtPrivateTime = (UNITS_IN_100NS / REF_TIME_MILLISECONDS) * m_dwPrevSystemTime;
...
STDMETHODIMP CBaseReferenceClock::GetTime(__out REFERENCE_TIME *pTime)
{
    ...
    if (pTime)
    {
        ...
        *pTime = m_rtLastGotTime;
        Unlock();
        MSR_INTEGER(m_idGetSystemTime, LONG((*pTime) / (UNITS_IN_100NS/REF_TIME_MILLISECONDS)) );
...
REFERENCE_TIME CBaseReferenceClock::GetPrivateTime()
{
    ...
    {
        m_rtPrivateTime += Int32x32To64(UNITS_IN_100NS / REF_TIME_MILLISECONDS, (DWORD)(dwTime - m_dwPrevSystemTime));
...
STDMETHODIMP CBaseReferenceClock::SetTimeDelta(const REFERENCE_TIME & TimeDelta)
{
    ...
    if (llDelta > UNITS_IN_100NS * 1000) {
