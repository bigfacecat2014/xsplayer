...
REFERENCE_TIME CAMSchedule::Advise( const REFERENCE_TIME & rtTime )
{
    ...
    DbgLog((LOG_TIMING, 2,
        TEXT("CAMSchedule::Advise( %lu ms )"), ULONG(rtTime / (UNITS_IN_100NS / REF_TIME_MILLISECONDS))));
    ...
    DbgLog((LOG_TIMING, 3,
            ...
            DWORD(rtNextTime / (UNITS_IN_100NS / REF_TIME_MILLISECONDS)), pAdvise->m_dwAdviseCookie ));
...
DWORD_PTR CAMSchedule::AddAdvisePacket( __inout CAdvisePacket * pPacket )
{
    ...
    DbgLog((LOG_TIMING, 2, TEXT("Added advise %lu, for thread 0x%02X, scheduled at %lu"),
    	pPacket->m_dwAdviseCookie, GetCurrentThreadId(), (pPacket->m_rtEventTime / (UNITS_IN_100NS / REF_TIME_MILLISECONDS)) ));
...
void CAMSchedule::ShuntHead()
{
    ...
    if (p_prev != pPacket)
    {
        ...
        DbgLog((LOG_TIMING, 2, TEXT("Periodic advise %lu, shunted to %lu"),
    	    pPacket->m_dwAdviseCookie, (pPacket->m_rtEventTime / (UNITS_IN_100NS / REF_TIME_MILLISECONDS)) ));
    #endif
    m_Serialize.Unlock();
}
...
void CAMSchedule::DumpLinkedList()
{
        ...
        )	
    {
        DbgLog((LOG_TIMING, 1, TEXT("Advise List # %lu, Cookie %d,  RefTime %lu"),
            i,
	    p->m_dwAdviseCookie,
	    p->m_rtEventTime / (UNITS_IN_100NS / REF_TIME_MILLISECONDS)
            ));
    }
    m_Serialize.Unlock();
}
