...
static const ADM_MAP_MESSAGE allMessages[] =
{
...
inline void WINAPI TraceMsg(LPCTSTR lpszPrefix, const MSG* pMsg)
{
	...
	else
	{
		...
		const ADM_MAP_MESSAGE* pMapMsg = allMessages;
	...
	if (lpszMsgName != NULL)
	{
		ATL::AtlTrace(_T("%Ts: hwnd=0x%08X, msg = %hs (0x%08X, 0x%08X)\n"),
			lpszPrefix, (DWORD)pMsg->hwnd, lpszMsgName,
			pMsg->wParam, pMsg->lParam);
	}
	else
	{
		ATL::AtlTrace(_T("%Ts: hwnd=0x%08X, msg = 0x%04X (0x%08X, 0x%08X)\n"),
			lpszPrefix, (DWORD)pMsg->hwnd, pMsg->message,
			pMsg->wParam, pMsg->lParam);
	}
}
