...
extern const UINT VIH2SIZE = _countof(vih2s);

CString VIH2String(int i)
{
    CString ret(GuidNames[*vih2s[i].subtype]);
    if (!ret.Left(13).CompareNoCase(_T("MEDIASUBTYPE_"))) {
        ret = ret.Mid(13);
    }
    if (vih2s[i].vih.bmiHeader.biCompression == 3) {
        ret += _T(" BITF");
    }
    if (*vih2s[i].subtype == MEDIASUBTYPE_I420) {
        ret = _T("I420");    // FIXME
    }
    return ret;
}
