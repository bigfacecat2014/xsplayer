...
Boolean MediaSession
::initiateByMediaType(char const* mimeType, MediaSubsession*& resultSubsession, int useSpecialRTPoffset) {
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
  ...
  do {
    ...
    if (rtpBufSize < RTP_RECV_BUF_SIZE)
      rtpBufSize = RTP_RECV_BUF_SIZE;
    ...
    do {
        ...
        while (*sdpLine != '\0' && *sdpLine != '\r' && *sdpLine != '\n') {
            ...
            if (sscanfResult >= 1) {
                ...
                if (sscanfResult == 1) {
                    setAttribute(nameStr); // <name>
                } else {
                    setAttribute(nameStr, valueStr); // <name>=<value>
                }
            }
    ...
    do {
        if (strcmp(fProtocolName, "UDP") == 0) {
            ...
            if (strcmp(fCodecName, "MPEG4-GENERIC") == 0) {
                fReadSource = MPEG4GenericRTPSource::createNew(env(), fRTPSocket,
                    fRTPPayloadFormat, fRTPTimestampFrequency, fMediumName,
                    attrVal_str("mode"),
	            ...
	            fReadSource = H265VideoRTPSource::createNew(env(), fRTPSocket,
                    fRTPPayloadFormat, expectDONFields, fRTPTimestampFrequency);
            ...
            if (createSimpleRTPSource) {
	            ...
	            fReadSource = SimpleRTPSource::createNew(env(), fRTPSocket, fRTPPayloadFormat,
                    fRTPTimestampFrequency, mimeType,
                    (unsigned)useSpecialRTPoffset,
                    doNormalMBitRule);
	            delete[] mimeType;
            }
            fRTPSource = (RTPSource*)fReadSource;
        } // end "TCP"
...
{
    if (strValue == NULL) {
        fIntValue = 1; // 猜测是bool类型，默认值假设为true
    }
    else {
        // Try to parse "strValue" as an integer.  If we can't, assume an integer value of 0:
        if (1 != sscanf(strValue, valueIsHexadecimal ? "%x" : "%d", &fIntValue)) {
            fIntValue = 0;
        }
    }
}
