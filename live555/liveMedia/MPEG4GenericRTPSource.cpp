...
{
  return new MPEG4GenericRTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency,
      mediumName, mode, sizeLength, indexLength, indexDeltaLength);
}

MPEG4GenericRTPSource::MPEG4GenericRTPSource(UsageEnvironment& env, Groupsock* RTPgs,
    unsigned char rtpPayloadFormat,
    unsigned rtpTimestampFrequency,
    char const* mediumName,
    char const* mode,
  ...
  fNumAUHeaders(0), fNextAUHeader(0), fAUHeaders(NULL) {
    ...
    if (mode == NULL)
        envir() << "MPEG4GenericRTPSource Warning: mode == NULL\n";
    else if (_stricmp(mode, "aac-hbr") != 0 && _stricmp(mode, "generic") != 0)
