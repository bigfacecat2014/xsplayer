...
::H264or5VideoStreamParser(int hNumber, H264or5VideoStreamFramer* usingSource,
  ...
  : MPEGVideoStreamParser(usingSource, inputSource),
    ...
    fHaveSeenFirstStartCode(False), fHaveSeenFirstByteOfNALUnit(False), fParsedFrameRate(0.0) {
}
...
::analyze_seq_parameter_set_data(unsigned& num_units_in_tick, unsigned& time_scale) {
  ...
  if (fHNumber == 264) {
	...
	// scaling_list_data()
	DEBUG_TAB;
    unsigned realSizeId = 0;
	for (unsigned sizeId = 0; sizeId < 4; ++sizeId) {
	  DEBUG_PRINT(sizeId);
      realSizeId = (sizeId == 3u ? 2u : 6u);
	  for (unsigned matrixId = 0; matrixId < realSizeId; ++matrixId) {
	    DEBUG_TAB;
